...
class SipHashTests {
    ...
    // SipHash-2-4 reference vectors: https://github.com/veorq/SipHash/blob/master/vectors.h
    ...
    @Test
    void matchesReferenceVectorsAcrossMessageBlockBoundaries() {
        byte[] input = new byte[VECTORS.length];
        for (int i = 0; i < input.length; i++) {
            input[i] = (byte) i;
        }
        for (int length = 0; length < input.length; length++) {
            assertEquals(VECTORS[length], SipHash.hash(input, length, 0x0706050403020100L, 0x0f0e0d0c0b0a0908L),
                    "Reference vector for " + length + " bytes");
        }
    }

    @Test
    void ipv6CookiesBindTheAddressAndPortAcrossKeyRotation() {
        long[] now = {599_999};
        SipHash hash = new SipHash(new byte[]{1, 2, 3, 4}) {
            @Override
            protected long now() {
                return now[0];
            }
        };
        InetSocketAddress sender = new InetSocketAddress("2001:db8::1", 19132);
        int cookie = hash.generateStatelessCookie(sender, 11);

        now[0] = 600_001;
        assertTrue(hash.validateCookie(cookie, sender, RakServerCookieMode.ACTIVE));
        assertTrue(hash.validateCookie(cookie, sender, RakServerCookieMode.OFFLOADED_PSK));
        assertEquals(11, SipHash.getProtocolVersion(cookie));
        assertFalse(hash.validateCookie(cookie, new InetSocketAddress("2001:db8::2", 19132), RakServerCookieMode.ACTIVE));
        assertFalse(hash.validateCookie(cookie, new InetSocketAddress("2001:db8::1", 19133), RakServerCookieMode.ACTIVE));

        now[0] = 660_000;
        assertFalse(hash.validateCookie(cookie, sender, RakServerCookieMode.ACTIVE));
    }
}
